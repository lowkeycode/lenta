...
async function getPhotos() {
    try{
        ...
        let lastImg = imageContainer.lastElementChild.previousElementSibling.firstChild;

        let offset = -lastImg + "px";
        ...
        const obsCallback = function (entries, observer) {
            ...
            //TODO Find out from the entries when to call getPhotos again (might be using rootMargin with offset in options instead of threshhold so it only goes one way?)

            // Get more photos

            // getPhotos();

            // Unobserve old last img
            lastImgObserver.unobserve(lastImg);

            // Reset old las img to new last img
            lastImg = imageContainer.lastElementChild.previousElementSibling.firstChild;
